...
const SinglePostScreen = (props) => {
  const [postid, setPostID] = useState(props.route.params.data);  
  ...
  const fetchData = async () => {
    const responseJson = await fetch_get(url1 +'/api/cmis/posts/'+postid.id);
    setPostObj(responseJson);
  }

  const fetchData2 = async () => {  
    const responseJson = await fetch_get(url1 +'/api/cmis/posts/'+postid.id+"/isLikedByStudent/"+ownID);
    setLiked(responseJson);

    const responseJson2 = await fetch_get(url1 +'/api/cmis/posts/'+postid.id+"/isBookmarkedByStudent/"+ownID);
  ...
  const handleBookmark = async (id) => {
    if(bookmarked==false){
      ...
      await fetch_delete(url1 +"/api/cmis/students/"+ownID+"/bookmarkedPosts/"+postid.id);
    }
  ...
  const handleLike = async () => {
    setLiked(!liked);
    const responseJson = await fetch_post(url1 +"/api/cmis/students/"+ownID+"/posts/"+postid.id+"/like",null);  
    setPostObj(responseJson);
  }; 
  ...
  return (
      ...
      <View style={{ paddingBottom:0,backgroundColor:'white', position:'absolute', width:'100%'}}>
          <View style={{height:height*0.06,flexDirection:'row', alignItems:'center', justifyContent:'center'}}> 
          ...
          {postObj && 
          <View style={{paddingBottom: 3,borderBottomColor: "rgba(208,210,242,0.2)",borderBottomWidth: 0,marginBottom:0}}>
                <View style={{backgroundColor:'rgba(217,217,217,1)',height:5}}>
                </View>
                  ...
                  <View style={{ flexDirection: "row", alignSelf: "flex-end" }}>
                    ...
                    <TouchableOpacity onPress={() => handleBookmark(postObj.id)} >
                      <MaterialCommunityIcons
                        ...
                        style={{ fontSize: 25, paddingRight: 5, paddingTop: 3, color: bookmarked ? "black" : "rgba(84,70,115,1)"}}
                      />
                    </TouchableOpacity>
                    }
